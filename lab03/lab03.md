...
### А. Однопоточный веб-сервер (3 балла) -- сделал
...
### Б. Многопоточный веб-сервер (2 балла) -- сделал
...
### Г. Ограничение потоков сервера (3 балла) -- сделал
...
#### Решение
Чтобы полететь дальше надо чтобы заполнился весь покет в 56 байт, для этого в теории может понадобиться подождать еще $56 \cdot 8 - 1$ битов для появления, т.е. $\dfrac {56 \cdot 8 - 1} {128 \cdot 1000} \approx 0.0034921875$. После чего пакет доходит за $\dfrac {56 \cdot 8} {10^6} + 0.005 \approx 0.005448$ и в итоге получается $\approx 0.0089401875$ секунд.


### Задача 2 (2 балла) -- сделал
...
#### Решение
$N = 11, d = 0.01 + \dfrac 1 {100} = 0.02 \implies a = \dfrac N d = \dfrac {11} {0.02} = 550$

т.е. 550 пакетов в секунду в среднем

### Задача 3 (2 балла) -- сделал первый пункт
Рассмотрим рисунок.
...
1. второй пакет сначала будет ждать пока первый пройдет по линии $R_S$ а после чего пройдет без помех(т.к. $R_S < R_C$), следовательно он будет ждать $\dfrac L {R_S}$ времени, а значит это и будет разница времен второго и первого пакетов. Ответ $\dfrac L {R_S}$.
