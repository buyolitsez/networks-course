...
#### Решение
Пакет не пойдет вперед пока стоит в очереди. Мы хотим понять за сколько времени дойдет самый последний пакет.

Т.е. он будет честно идти свои $N \dfrac L R$ и при этом ждать остальных $(P - 1) \dfrac L R$, т.е. суммарно $N \dfrac L R + (P - 1) \dfrac L R$.
...
#### Решение
Пропускная способность такой сети равна минимуму пропускных способностей на ребрах, т.е. 200 Кбит/с, тогда итоговое время $\dfrac {5 \cdot 1024} {200} = 25.6$ секунд.
...
Вероятность одновременной у 12 или более равна $\sum_{i = 12}^{60} \binom {60} {i} 0.2^i \cdot 0.8^{60 - i}\approx 0.55$.
...
1. задержка передачи равна $\dfrac L R$, тогда итоговая задержка это $\dfrac L R + \dfrac {I \cdot L} {R (1 - I)} = \dfrac {L} {R (1 - I)}$
2. пусть $\dfrac L R = b$, тогда значем что $I = ab$, тогда общая задержка это $\dfrac b {1 - ab}$
